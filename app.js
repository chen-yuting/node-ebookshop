...
app.get('/book/home', (req, res) => {
    ...
    conn.query('select * from book where cover != \'\'', (err, results) => {
        ...
        const guessYouLike = []
        const img = Math.floor(Math.random() * 10) % 2 == 0 ? '/home_banner2.jpg' : '/home_banner.jpg'
        const banner = constant.resUrl + img
        const recommend = []
        const featured = []
        const random = []
...
function createGuessYouLike(data) {
    ...
    switch (n) {
        case 1:
            data['result'] = data.id % 2 == 0 ? '《Executing Magic》' : '《Elemeents Of Robotics》'
            break;
        case 2:
            data['result'] = data.id % 2 == 0 ? '《Improve Psychiatric Care》' : '《Programming Languages》'
            break;
        case 3:
